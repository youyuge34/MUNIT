...
class MUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        self.gen_opt = torch.optim.Adam([p for p in gen_params if p.requires_grad],
                                        lr=lr, betas=(beta1, beta2), weight_decay=hyperparameters['weight_decay'])
        # 若resume，则会重新恢复scheduler。train里会scheduler.step()
        self.dis_scheduler = get_scheduler(self.dis_opt, hyperparameters)
        self.gen_scheduler = get_scheduler(self.gen_opt, hyperparameters)

        # Network weight initialization
        # model.apply(fx)  ---->  fx(module.childrens) 会将fx函数递归调用到每个子module上
        # 所以weights_init 返回的是一个 内部方法
...
class MOUNT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(MOUNT_Trainer, self).__init__()
        lr = hyperparameters['lr']
        # Initiate the networks
        self.gen_a = AdaINGen(hyperparameters['input_dim_a'], hyperparameters['gen'])  # auto-encoder for domain a
        self.gen_b = AdaINGen(hyperparameters['input_dim_b'], hyperparameters['gen'])  # auto-encoder for domain b
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        '''
        修改后：x_a uses decode, x_b uses decode_content_only
        '''
        self.gen_opt.zero_grad()
        s_a = Variable(torch.randn(x_a.size(0), self.style_dim, 1, 1).cuda())
        s_b = Variable(torch.randn(x_b.size(0), self.style_dim, 1, 1).cuda())
        ...
        # decode (within domain)
        x_a_recon = self.gen_a.decode(c_a, s_a_prime)
        x_b_recon = self.gen_b.decode_content_only(c_b)
        # decode (cross domain)
        x_ba = self.gen_a.decode(c_b, s_a)
        x_ab = self.gen_b.decode_content_only(c_a)
        # encode again
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            c_b, s_b_fake = self.gen_b.encode(x_b[i].unsqueeze(0))
            x_a_recon.append(self.gen_a.decode(c_a, s_a_fake))
            x_b_recon.append(self.gen_b.decode_content_only(c_b))
            x_ba1.append(self.gen_a.decode(c_b, s_a1[i].unsqueeze(0)))
            x_ba2.append(self.gen_a.decode(c_b, s_a2[i].unsqueeze(0)))
