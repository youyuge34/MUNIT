...
parser.add_argument('--output_folder', default="results/", type=str, help="output image path")
...
parser.add_argument('--trainer', type=str, default='MOUNT', help="MOUNT only！！！")
...
with torch.no_grad():
    ...
    if opts.trainer == 'MOUNT':
        ...
        outputs = (outputs + 1) / 2.
        path = os.path.join(opts.output_folder, 'output_{}_{}_{}.jpg'.format(opts.trainer,opts.input.split("\\")[-1].split('.')[0], time.strftime("%H_%M_%S", time.localtime())))
